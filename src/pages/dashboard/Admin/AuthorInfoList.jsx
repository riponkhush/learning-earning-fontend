    ...
    const { refetch, data: users = [] } = useQuery({
        ...
        queryFn: async () => {
            ...
            const filteredData = sortedData.filter(user => 
                ...
                user.author_email?.toLowerCase().includes(searchTerm.toLowerCase())
            );
    ...
    const handleDelete = (id) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axiosPublic.delete(`/authorInfo/${id}`).then((res) => {
                ...
                <Card>
                    <CardHeader className="mb-8 md:p-6 p-3 bg-blue-500">
                        <div className='flex justify-between items-center'>
                            <Typography variant="h6" color="white">Author info List</Typography>
                            ...
                            <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
                                {
                                    users.length > 0 ? (
                                                ...
                                                <tr key={item._id}>
                                                    <td className="py-3 px-5">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <Typography variant="small" color="blue-gray" className="font-semibold">{item.author_name}</Typography>
                                                                <Typography className="text-xs font-normal text-blue-gray-500">{item.author_email}</Typography>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="py-3 px-5">
                                                        <Typography className="text-xs font-semibold text-blue-gray-600">{item.dasignation}</Typography>
                                                    </td>
                                                    <td className="py-3 px-5">
                                                        <Chip
                                                            variant="gradient"
                                        ...
                                        <Typography className="text-center text-4xl text-blue-gray-500 flex justify-center items-center h-96">You have no Author.</Typography>
                                    )
                                }
                        </CardBody>
                        )
