...
const AddminHomeworkList = () => {
    ...
    const { refetch, data: isCourses = [] } = useQuery({
        queryKey: ["isCourses", active, searchTerm],
        queryFn: async () => {
            ...
            const filteredData = sortedData.filter(user => 
                user.homework?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleDelete = (id) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axiosPublic.delete(`/studentHomework/${id}`).then((res) => {
                    ...
                    <Card>
                        <CardHeader className="mb-8 md:p-6 p-3 bg-blue-500">
                            <div className='flex justify-between items-center'>
                                <Typography variant="h6" color="white">All homework List</Typography>
