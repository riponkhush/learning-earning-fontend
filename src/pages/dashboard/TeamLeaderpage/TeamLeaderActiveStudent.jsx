...
const TeamLeaderActiveStudent = () => {
    ...
    const {  data: users = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosPublic.get(`/teamLeader?user_email=${user.email}`)
    ...
    const { refetch, data: tainers = [] } = useQuery({
        queryKey: ["tainers", user?.email],
        queryFn: async () => {
    ...
    const handlePostToLeader = () => {
        ...
        axiosPublic.post('/TraninerData', { trainerEmail: selectedLeader, studentId: selectedUsers , loggedInUserEmail: user.email })
            .then(response => {
                        ...
                        <div className='flex justify-between gap-1 items-center md:py-3 md:px-5 px-2'>
                                    <div className="md:w-72">
                                        <Select className='' label="Select Trainer" onChange={(value) => handleLeaderChange(value)} >
                                            {tainers.map(item => <Option className='text-black' key={item._id} value={item.email}>{item.name} trainer
